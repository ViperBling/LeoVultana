...
auto HelloTriangleSample::destroyGraphicsPipeline() -> void
{
    ...
    vkDestroyPipeline(mDevice.GetDevice(), graphicsPipeline, nullptr);
...
auto HelloTriangleSample::createGraphicsPipeline() -> void
{
    ...
    if (vkCreatePipelineLayout(mDevice.GetDevice(), &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS)
    {
    ...
    if (vkCreateGraphicsPipelines(mDevice.GetDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS)
    {
...
void HelloTriangleSample::OnCreate()
{
    ...
    m_CommandListRing.OnCreate(&mDevice, backBufferCount, commandListsPerBackBuffer);
...
void HelloTriangleSample::OnRender()
{
    ...
    BeginFrame();
    int imageIndex = mSwapChain.WaitForSwapChain();
    ...
    // hello triangle draw
    {
        ...
        renderPassInfo.framebuffer = mSwapChain.GetCurrentFrameBuffer();
    ...
    mSwapChain.GetSemaphores(&ImageAvailableSemaphore, &RenderFinishedSemaphores, &CmdBufExecutedFences);
    ...
    if (vkQueueSubmit(mDevice.GetGraphicsQueue(), 1, &submitInfo, CmdBufExecutedFences) != VK_SUCCESS)
    {
