    ...
    VkSampleCountFlagBits sampleDescCount)
{
    ...
    viBindingDesc.stride = sizeof(float) * 3;
    ...
    SetResourceName(pDevice->GetDevice(), VK_OBJECT_TYPE_PIPELINE, (uint64_t)mPipeline, "Wireframe P");
}
...
void Wireframe::Draw(
    ...
    uint32_t numIndices,
    VkDescriptorBufferInfo IBV, VkDescriptorBufferInfo VBV,
    ...
    const math::Vector4 &vColor)
{
    ...
    vkCmdBindVertexBuffers(cmdBuffer, 0, 1, &VBV.buffer, &VBV.offset);
    vkCmdBindIndexBuffer(cmdBuffer, IBV.buffer, IBV.offset, VK_INDEX_TYPE_UINT16);
    ...
    cbPerObject->mWorldViewProj = worldMatrix;
    cbPerObject->mCenter = vCenter;
    cbPerObject->mRadius = vRadius;
    cbPerObject->mColor = vColor;
