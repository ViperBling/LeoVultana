        ...
        if (GetExitCodeProcess(pi.hProcess, &rc))
        {
            ...
            else
            {
                ...
                for (;;)
                {
                    DWORD dwRead;
                    char chBuf[2049];
                    BOOL bSuccess = ReadFile(g_hChildStd_OUT_Rd, chBuf, 2048, &dwRead, nullptr);
                    chBuf[dwRead] = 0;
...
{
    OVERLAPPED* pOverlapped = &mOverlappedData[mCurrentIOBufferIndex];

    // Make sure any previous write with this overlapped structure has completed
    DWORD numTransferedBytes;
    GetOverlappedResult(mFileHandle, pOverlapped, &numTransferedBytes, TRUE);  // This will wait on the current thread

    // Apply increments accordingly
    pOverlapped->Offset = mWriteOffset;
    mWriteOffset += static_cast<uint32_t>(strlen(LogString));

    mCurrentIOBufferIndex = (++mCurrentIOBufferIndex % MAX_INFLIGHT_WRITES);  // Wrap when we get to the end

    bool result = WriteFileEx(mFileHandle, LogString, static_cast<DWORD>(strlen(LogString)), pOverlapped, OverlappedCompletionRoutine);
    assert(result);
}